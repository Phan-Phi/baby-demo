...
export default function GetYoutubeThumbnail({ data }: Props) {
  const youtubeThumbnail = require("youtube-thumbnail");
  const router = useRouter();
  const { isMdDown } = useMedia();
  const { messages } = useIntl();
  const thumbnail = youtubeThumbnail(data);

  return (
    <Wrapper>
      <WrapperVideo flexGrow={1}>
        <Image src={thumbnail.high.url} alt="" style={{ borderRadius: "5px" }} />
      </WrapperVideo>
